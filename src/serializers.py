...
class SequenceContext(str, Enum):
    """
    Contexte d'une séquence de données (montée, descente, marche, apnée, repos).
    """
    MONTEE = "MONTEE"
    DESCENTE = "DESCENTE"
    MARCHE = "MARCHE"
...
class SequenceOutput(BaseModel):
    """
    Modèle représentant une séquence de données physiologiques, avec des méthodes pour le traitement et la visualisation.
    """
    deviceId: str
    sequenceId: int
    ...
    def dataframe(self) -> pd.DataFrame:
        """
        Convertit les données brutes en DataFrame Pandas avec un index temporel et des colonnes calculées.
        
        Returns:
            pd.DataFrame: DataFrame contenant les données de la séquence avec des colonnes supplémentaires
        """
    ...
    def simple_line_plot(self, channel: str, timestamp: bool = False) -> go.Figure:
        """
        Trace un graphique simple du signal pour un canal donné.
        Args:
            channel (str): Le canal à tracer (ex: "RESP_THORAX").
            timestamp (bool): Si True, utilise l'axe temporel; sinon, utilise l'index de base.
        Returns:
            go.Figure: La figure Plotly du signal tracé.
        """
        fig = go.Figure()
        df = self.dataframe
        if timestamp:
    ...
    def simple_line_plot_with_peaks(self, channel: str = "RESP_THORAX", prominence: float = 8, timestamp: bool = False) -> go.Figure:
        """
        Trace le signal pour un canal donné avec détection et marquage des pics.
        
        Args:
            channel (str): Le canal à tracer (ex: "RESP_THORAX").
            prominence (float): La proéminence minimale des pics à détecter.
            timestamp (bool): Si True, utilise l'axe temporel; sinon, utilise l'index de base.
        Returns:
            go.Figure: La figure Plotly du signal tracé avec les pics marqués.
        """
        fig = go.Figure()
        df = self.dataframe
        if timestamp:
    ...
    def get_number_of_peaks(self, channel: str = "RESP_THORAX", prominence: float = 8) -> int:
        """
        Retourne le nombre de pics détectés dans un canal donné.
        
        Args:
            channel (str): Le canal à analyser (ex: "RESP_THORAX").
            prominence (float): La proéminence minimale des pics à détecter.
        Returns:
            int: Le nombre de pics détectés.
        """
        df = self.dataframe
        peaks, _ = find_peaks(df[channel], prominence=prominence)
        return len(peaks)

    def fourier_transform_plot(self, channel: str = "RESP_THORAX") -> go.Figure:
        """
        Trace la transformée de Fourier du signal pour un canal donné.
        
        Args:
            channel (str): Le canal à analyser (ex: "RESP_THORAX").
        Returns:
            go.Figure: La figure Plotly de la transformée de Fourier.
        """
    ...
    def local_average_plot(self, channel: str = "RESP_THORAX", window_size_seconds: float = 5) -> go.Figure:
        """
        Trace le signal pour un canal donné avec une moyenne locale glissante.
        
        Args:
            channel (str): Le canal à analyser (ex: "RESP_THORAX").
            window_size_seconds (float): La taille de la fenêtre de la moyenne locale en secondes.
        Returns:
            go.Figure: La figure Plotly du signal avec la moyenne locale.
        """
...
class SequenceInput(SequenceOutput):
    """
    Étend SequenceOutput pour inclure des métadonnées supplémentaires nécessaires à l'envoi au serveur.
    """
...
class SessionHeader(BaseModel):
    """
    En-tête d'une session, contenant les métadonnées techniques.
    """
    sampling_rate: int
...
class SequenceMetadata(BaseModel):
    """
    Métadonnées d'une séquence au sein d'une session.
    """
    sequenceId: int
    begin: int
    end: int
...
class SessionMetadata(BaseModel):
    """
    Métadonnées d'une session, incluant la liste des séquences.
    """
    studentId: str
...
class SessionInput(BaseModel):
    """
    Représente une session complète avec ses séquences, pour l'envoi au serveur.
    """
    studentId: str # studentId privé, pour l'envoi au serveur
    sessionId: str
...
class SessionOutput(BaseModel):
    """
    Représente une session complète avec ses séquences, pour la lecture depuis le serveur.
    """
    ...
    def get_descriptions(self) -> list[str]:
        """
        Retourne la liste des descriptions des séquences de la session.
        Returns:
            list[str]: Liste des descriptions des séquences.
        """
        return [seq.sequenceDescription for seq in self.sequences]

    def plot(self, channel: str, timestamp: bool = False) -> plt.Figure:
        """
        Trace les signaux d'un canal donné pour toutes les séquences de la session, en les concaténant.
        
        Args:
            channel (str): Le canal à tracer (ex: "RESP_THORAX").
            timestamp (bool): Si True, utilise l'axe temporel; sinon, utilise l'index de base.
        Returns:
            plt.Figure: La figure Matplotlib des signaux concaténés.
        """
        color_map = {
